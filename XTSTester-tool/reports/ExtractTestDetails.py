...
def parse_html_report(html_file):
    ...
    返回:
        包含测试结果的字典，格式与extract_test_details相同
    """
    if not os.path.exists(html_file):
        raise FileNotFoundError(f"HTML报告文件不存在: {html_file}")
    ...
    # 解析HTML报告中的测试结果
    # 假设HTML报告中有以下结构:
    # 1. 测试摘要表格包含总测试数、通过数、失败数
    # 2. 每个测试类有自己的表格，包含测试方法、状态和耗时
    
    # 解析测试摘要
    summary_table = soup.find('table', {'class': 'summary'})
    if summary_table:
    ...
    # 解析测试类表格
    class_tables = soup.find_all('table', {'class': 'test-class'})
    for table in class_tables:
        # 获取测试类名
        class_name = table.find_previous('h3').text.strip()
        test_results[class_name] = []
        class_time = 0
        ...
        for row in rows:
            ...
            if len(cells) >= 3:
                ...
                # 检查是否有错误信息
                if len(cells) >= 4 and cells[3].text.strip():
                    test_info['error_stack'] = cells[3].text.strip()
    ...
    # 计算总耗时
    summary['total_time_ms'] = sum(class_times.values())
...
def save_test_json(test_results, summary, class_times, output_file="test_results.json", source_type="text"):
    """
    将测试结果保存为JSON文件
    ...
    参数:
        ...
        class_times: 测试类耗时字典
        output_file: 输出文件路径
        source_type: 数据来源类型 ("text" 或 "html")
    """
    # 创建一个包含所有信息的字典
    data = {
        "summary": summary,
