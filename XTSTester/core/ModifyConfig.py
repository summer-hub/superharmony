        ...
        if current_library:
            ...
            if owner:
                ...
                if "openharmony-sig" in owner.lower():
                    ...
                    # 进一步区分是tpc还是samples
                    if "openharmony_tpc_samples" == name.lower():
                        repo_type = "samples"
                ...
                try:
                    ...
                    if matched_libraries:
                        ...
                        owner, name, _ = get_repo_info(matched_lib)
                        if owner:
                            if "openharmony-sig" in owner:
                                ...
                                if "openharmony_tpc_samples" == name:
                                    repo_type = "samples"
                                    signing_config = SIGNING_CONFIG_SAMPLES
                                    bundle_name = BUNDLE_NAME_SAMPLES
                                else:
                                ...
                                print(f"无法模糊匹配仓库类型: {current_library}")
                except Exception as e:
                    print(f"模糊匹配库时出错: {str(e)}")
            else:
                print(f"无法确定仓库类型: {current_library}")
...
def _get_current_library_name():
    ...
    try:
        ...
        libraries, _, _ = read_libraries_from_excel()
        for lib in libraries:
            _, name, sub_dir = get_repo_info(lib)
            # 检查仓库名是否匹配
